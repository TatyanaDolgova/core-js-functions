...
function logger(func, logFunc) {
  return (...args) => {
    ...
    const newString = argsStringify.slice(1, argsStringify.length - 1);
    const stringStart = `${func.name}(${newString}) starts`;
    logFunc(stringStart);
    const result = func(...args);
    const stringEnd = `${func.name}(${newString}) ends`;
    logFunc(stringEnd);
    return result;
  };
}
